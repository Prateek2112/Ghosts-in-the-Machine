...
# Function for agent 4 without information of ghosts inside walls
def agent_4_low_info():
    ...
    while c.AGENT_POS != c.END:  # Loop till agent does not reach end cell
        ...
        if is_alive:
            ...
            if path:
                cell = path[-1]
                path.remove(cell)
                is_alive = m.ghost_movement_low_info(c.GRID, c.AGENT_POS, c.GHOST_POS, low_info_ghost_pos, path)
                ...
                if any(gp in path for gp in ghost_near) or any(p in path for p in low_info_ghost_pos) is True:
                    for i in path:
                ...
                is_alive = is_alive and m.ghost_movement_low_info(c.GRID, c.AGENT_POS, c.GHOST_POS, low_info_ghost_pos, [])
